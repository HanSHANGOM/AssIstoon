# -*- coding: utf-8 -*-
"""marcnn train test colab ver.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1zDtmrxryeDmPYJ9wSxtcuqPkdvr1fiqR
"""
...
#!unzip '/content/drive/My Drive/mrcnn.zip'
...
#COCO_WEIGHTS_PATH = os.path.join('/content/dataset/logs/manga20190728T0623', "mask_rcnn_manga_0040.h5")

COCO_WEIGHTS_PATH = os.path.join('/content/dataset/', "mask_rcnn_coco.h5")
...
VAL_IMAGE_IDS = [name.split('.')[0] for name in os.listdir('/content/dataset/val/images/')]
print(VAL_IMAGE_IDS)
...
class MangaDataset(utils.Dataset):
    ...
    def load_Manga(self,dataset_dir,subset):
        """망가데이터셋의 서브셋 불러오기.
    ...
    def load_mask(self, image_id):
       ...
       반환값:
        ...
        mask_dir = os.path.join('/content/dataset/train/', "masks")
        # png이미지로부터 mask파일을 읽는다.
        mask_list = [f for f in os.listdir(mask_dir) if f.split('_')[0]== info['id'].split('_')[0]+info['id'].split('_')[1] and f.endswith(".png")]
        #mask=np.zeros(skimage.io.imread(os.path.join(mask_dir, mask_list[0])).shape,dtype=np.int32)
        mask=[]
        for f in mask_list:
...
def train(model, dataset_dir="/content/dataset/"):
    """Train the model."""
    # Training dataset.
